...
def preencherCampos(event) -> None:
    ...
    txtIdade.config(state=DISABLED)
    comboCursos.set(aluno["curso"])
    comboCursos.config(state=DISABLED)

def editarAluno() -> None:
    opcaoSelecionada = messagebox.askyesno("Tem Certeza", "Deseja altera os dados?")
    if opcaoSelecionada:
        txtNome.config(state=NORMAL)
        txtIdade.config(state=NORMAL)
        comboCursos.config(state=NORMAL)
...
btnSalvar = Button(janela, text="Salvar", font="Tahoma 16", fg="red",
                    height=1, command=salvarAluno, state=DISABLED)
btnSalvar.grid(row=4,column=2)
